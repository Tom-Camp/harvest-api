...
from app.utils.database import create_db_and_tables, engine, init_casbin_tables
...
async def setup_initial_admin(session: AsyncSession):
    admin_data = {
        "username": settings.INITIAL_USER_NAMEIn,
    ...
    existing_admin = await UserCRUD.get_user_by_username(
        session, admin_data["username"]
    )
    if not existing_admin:
        admin_create = await UserCreate(**admin_data)
...
async def initialize_data():
    ...
    with AsyncSession(engine) as session:
        ...
        logger.info(f"Admin user created: username={admin.username}")
