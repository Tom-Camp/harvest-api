import time
from typing import Sequence
from uuid import UUID

from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy.orm import selectinload
from sqlmodel import select

from app.logging import get_logger, log_handler
from app.models.garden_models import GardenNote
from app.schemas.garden_note_schemas import GardenNoteCreate, GardenNoteUpdate

logger = get_logger(__name__)


class GardenNoteService:

    def __init__(self, session: AsyncSession):
        self._db = session

    async def create_note(self, note: GardenNoteCreate) -> GardenNote:
        """
        Create a new GardenNote

        :param note: A GardenNoteCreate object; gardens.garden_note_schemas.GardenNoteCreate
        :return: GardenNote; garden.garden_models.GardenNote
        """

        new_note = GardenNote(**note.model_dump())
        start = time.time()

        self._db.add(new_note)
        await self._db.commit()
        await self._db.refresh(new_note)

        duration_ms = (time.time() - start) * 1000
        log_handler.log_database_operation(
            operation="create_note",
            table="garden_note",
            duration_ms=duration_ms,
            note_id=str(new_note.id),
        )
        return new_note

    async def get_note(self, note_id: UUID) -> GardenNote | None:
        """
        Get a GardenNote by ID

        :param note_id: The ID of the GardenNote to retrieve
        :return: GardenNote | None; garden.garden_models.GardenNote
        """

        statement = select(GardenNote).where(GardenNote.id == note_id)

        start = time.time()

        result = await self._db.execute(statement)
        note = result.scalars().first()

        duration_ms = (time.time() - start) * 1000
        log_handler.log_database_operation(
            operation="get_note",
            table="garden_note",
            duration_ms=duration_ms,
            note_id=note_id,
        )
        return note

    async def get_notes(
        self, garden_id: UUID, skip: int = 0, limit: int = 100
    ) -> Sequence[GardenNote]:
        """
        Get all GardenNotes by ID

        :param garden_id: The ID of the GardenNote to retrieve
        :param skip: The number of rows to skip
        :param limit: The number of rows to return
        :return: Sequence[GardenNote]; garden.garden_models.Garden
        """

        statement = (
            select(GardenNote)
            .where(GardenNote.garden_id == garden_id)
            .offset(skip)
            .limit(limit)
        )

        start = time.time()

        result = await self._db.execute(statement)
        notes = result.scalars().all()

        duration_ms = (time.time() - start) * 1000
        log_handler.log_database_operation(
            operation="get_notes",
            table="garden_note",
            duration_ms=duration_ms,
            garden_id=str(garden_id),
        )
        return notes

    async def update_note(
        self, note_id: UUID, note_update: GardenNoteUpdate
    ) -> GardenNote | None:
        """
        Update a GardenNote by ID

        :param note_id: The ID of the GardenNote to update
        :param note_update: The GardenNoteUpdate object; gardens.garden_note_schemas.GardenNoteUpdate
        :return: GardenNote; gardens.garden_models.GardenNote
        """

        note = await self._db.get(
            GardenNote, note_id, options=[selectinload(GardenNote.garden)]
        )
        if note:
            note_data = note_update.model_dump(exclude_unset=True)
            for field, value in note_data.items():
                setattr(note, field, value)
            start = time.time()

            self._db.add(note)
            await self._db.commit()
            await self._db.refresh(note)

            duration_ms = (time.time() - start) * 1000
            log_handler.log_database_operation(
                operation="update_note",
                table="garden_note",
                duration_ms=duration_ms,
                garden_id=str(note_id),
            )
        return note

    async def delete_note(self, note_id: UUID) -> bool:
        """
        Delete a GardenNote by ID

        :param note_id: The ID of the GardenNote to delete
        :return: boolean
        """

        return_value: bool = False

        note = await self._db.get(GardenNote, note_id)
        if note:
            start = time.time()

            await self._db.delete(note)
            await self._db.commit()
            return_value = True

            duration_ms = (time.time() - start) * 1000
            log_handler.log_database_operation(
                operation="delete_note",
                table="garden_note",
                duration_ms=duration_ms,
                note_id=str(note_id),
            )
        return return_value
