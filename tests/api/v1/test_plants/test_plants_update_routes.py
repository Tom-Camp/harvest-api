...
class TestPlantUpdateRoutes:
    ...
    async def test_update_plant_any(
        ...
        if isinstance(garden, Garden):
            new_bed = await create_bed(
                client=client, garden_id=str(garden.id), headers=create_headers
            )
            plant = await create_plant(
                client=client, bed_id=str(new_bed.get("id", "")), headers=create_headers
            )
    ...
    async def test_update_plant_own(
        ...
        if isinstance(garden, Garden):
            ...
            plant = await create_plant(
                client=client, bed_id=str(new_bed.get("id", "")), headers=headers
            )
            plant_id = plant.get("id")
            response = await client.put(
                url=f"/api/plants/{plant_id}",
