...
from tests.helpers.test_helpers import create_bed, create_note, get_auth_headers
...
class TestBedNoteReadRoutes:
    ...
    async def test_read_bed_note(
        ...
        if isinstance(garden, Garden):
            ...
            self.note_json["bed_id"] = str(new_bed.get("id", ""))
    ...
    async def test_read_bed_note_unauthenticated(
        ...
        post_headers = await get_auth_headers(client=client, user_name="test_user")
        garden = default_gardens.get("test_user")
        if isinstance(garden, Garden):
            new_bed = await create_bed(
                client=client, garden_id=str(garden.id), headers=post_headers
            )
            self.note_json["bed_id"] = str(new_bed.get("id", ""))
            new_note = await create_note(
                client=client,
    ...
    async def test_read_bed_notes(
        ...
        if isinstance(garden, Garden):
            ...
            response = await client.get(
                url=f"/api/bed-notes/notes/{new_bed.get('id', '')}",
                headers=headers,
            )
            assert response.status_code == expected_status
